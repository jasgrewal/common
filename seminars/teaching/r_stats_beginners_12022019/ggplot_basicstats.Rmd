...
We will be using a package called [ggpubr](https://cran.r-project.org/web/packages/ggpubr/index.html) today.  
...
Over 50% of the genes across 5 samples are missing. We can deal with this either using imputation strategies, or by discarding the problematic samples. As imputation strategies are an entire discussion by themselves, we will into dive into them today (additional resources available at end of tutorial). Instead, we will take the easy way out and remove the samples with NAs. Good thing we have already covered a quick way to unselect these samples!.    

```{r, eval=TRUE, echo=TRUE}
data_df_clean <- data_df[complete.cases(data_df), ]
```

After this, we have `nrow(data_df_clean)` samples and `ncol(data_df_clean)` genes. We can do a quick 'smell-test' on this data, by using the dataframe function `summary()`. This function calculates summary statistics for each column in the dataframe. We can transpose the dataframe so that the samples become columns (instead of rows).  
...
ggplot has a handy geom_object (remember these from the ggplot tutorial?). The `stat_summary()` method allows us to plot a pointrange plot showing the mean and standard error.  
...
However, we have an anomalous sample! The sample `PDX1328` has readings that lie extremely outside the range of the rest of the samples. We can see this more clearly with a boxplot version of the datapoints, plotted using `geom_boxplot()`
```{r, eval=TRUE, echo=TRUE, fig.width=6, fig.height=7}
...
```{r, eval=TRUE, echo=TRUE}
kable(head(metadata_df))
```

`PDX1258` is a breast carcinoma. We can see the different disease categories by summarizing the contents of the column `cl_disease_detail`.  
```{r, eval=FALSE, echo=TRUE}
table(metadata_df$cl_disease_detail)
```
```{r, eval=TRUE, echo=FALSE}
kable(table(metadata_df$cl_disease_detail))
```
...
For the last bit, we will use an in-built dataset, `airquality`. You can laod it into your current environment by typing `data(airquality)`.  
```{r, eval=TRUE, echo=TRUE}
data(airquality)
```
```{r, eval=TRUE, echo=FALSE}
...
This is because the categories we are passing to perform the paired test for significance is numeric (hence 'continuous'). We can overcome this by treating the category column's values (Month) as strings. 
...
You can also calculate the significance of difference in means between all groups relative to a reference, like so:  
