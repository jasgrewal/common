...
We can remove the outlier sample using the following command (note that we are making changes to the samplxe x gene dataframe, not the melted version). For the sake of this tutorial, we have already decided that this is a **technical outlier**, whereby the readings themselves are in an entirely different range from the rest of the data. Had it been a biological outlier, you may want to keep it in your data!       
