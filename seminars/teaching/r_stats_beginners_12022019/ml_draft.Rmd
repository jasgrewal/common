---
title: "I want to do some machine learning on my data - introductory tutorial"
...
# Getting started  
This tutorial assumes you know how to load data into an RStudio session, view a dataframe and explore columns/rows of a dataframe in R. Knowing how to visualize data as scatterplots with ggplot2 will also be helpful, although not essential.      

We will be using 2 key packages today - []() and [caret](http://caret.r-forge.r-project.org/).  

```{r opts, eval=FALSE}

```

Let us load up ggplot2 into our current environment, just in case we want to make pretty plots.   
```{r, eval=TRUE}
...
Typically, when we want to use machine learning for tasks, we wish to automate workflows. 
The easiest way to know when a machine learning solution is feasible for your problem is when you already have a sense of some rules/input values that are predictive of certain outcomes. For example, let us look at the iris dataset. 

(Plot features on different axes, color points by class)  
(SEE: https://www.datacamp.com/community/tutorials/machine-learning-in-r#r )

How do we go about automating this separation? For this, we need to define what our input is, and what our output should be. Machine learning is a fancy way of saying, I have determined a mathematical formula that gives a numeric output, when given some numeric input(s).   

(We equate output labels with 'numbers')
(Example on converting continuous data to discrete, changing label outputs to numeric form) 

For the sake of this tutorial, we will assume we already know what data we are using to achieve this.  
...
(Data normalization - when the data is not consistent)
## What is my approach?  

Before we start trying to answer our question with the dataset at hand, we must appreciate the various algorithms we have the option of choosing from.  

### Do I have enough data?  

**Exploring the impact of training data on performance.**   
After we have determined what our question is and what our expected output from the data is, the next question usually is if the data we have is enough?  
The answer is dependent on the question you are trying to address. If your question is very complex, and you are trying to identify rare events/outcomes, your training data will need to have enough examples so that a machine learning algorithm can learn to distinguish outcome-specific patterns.  
In this section, we will explore how the performance of an algorithm changes, when  
(a) drastically different number of examples are used for training.   
(b) drastically different proportions of outcome-examples are used for training.
(c) different features are used for training. (weak features vs clear features)

```{r, eval=TRUE, echo=TRUE, fig.width=6, fig.height=5.5}
# Get data we were working with earlier  
data("msleep", package = "ggplot2")
# Plot the ggplot  
ggplot(msleep, aes(x=vore, y=sleep_rem)) + geom_point(alpha=0.5) + stat_summary() + theme_minimal()   + theme(text = element_text(size=12)) + ggtitle("I changed the title") + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 45, hjust = 1)) + labs(x="-vore", y="REM sleep") + facet_wrap(~conservation, ncol=3)
```
...
```{r, eval=TRUE, echo=TRUE, fig.width=7, fig.height=6}
ggplot(midwest, aes(x=state, y=log2(popdensity))) + geom_boxplot() + geom_dotplot(binaxis="y",stackdir="center", alpha=0.3, dotsize = 0.3) + theme_bw()
```

Another way to do it is using the [ggbeeswarm package](https://github.com/eclarke/ggbeeswarm), if you want other specialized types of 'swarms'.   

You can also learn about other cool types of ggplot functions and figures [using this cheat sheet](https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf)  
